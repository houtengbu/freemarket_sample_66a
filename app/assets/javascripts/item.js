...
$(function(){
  $('#price_calc').on('input', function(){   //リアルタイムで表示したいのでinputを使う｡入力の度にイベントが発火するようになる｡
    ...
    $('.tem-profit-title-box-a-text-right_bar_1').html(fee) //  手数料の表示｡html()は追加ではなく､上書き｡入力値が変わる度に表示も変わるようにする｡
    $('.tem-profit-title-box-a-text-right_bar_1').prepend('¥') // 手数料の前に¥マークを付けたいので
    $('.item-profit-title-box-b-text-right_bar_2').html(profit)
    $('.item-profit-title-box-b-text-right_bar_2').prepend('¥')
    $('#price').val(profit) // 計算結果を格納用フォームに追加｡もし､入力値を追加したいのなら､今回はdataを引数に持たせる｡
    if(profit == '') {   // もし､計算結果が''なら表示も消す｡
    $('.item-profit-title-box-b-text-right_bar_2').html('');
    $('.tem-profit-title-box-a-text-right_bar_1').html('');
    }
  })
})
